...
// Starter represents the aggregate root for employee management.
type Starter struct {
	id             int64
	domain         string
	name           string
...
// RehydrateStarter reconstructs a Starter aggregate from persistence.
func RehydrateStarter(id int64, domain, name, email, mobile, workPhone, jobTitle string, departmentID, lineManagerID *int64, createdAt, updatedAt time.Time) *Starter {
	return &Starter{
		id:            id,
		domain:        domain,
		name:          name,
...
// UpdateInfo updates the mutable fields of the starter.
...
// AssignToDepartment assigns starter to a department
func (s *Starter) AssignToDepartment(departmentID int64) {
	s.departmentID = &departmentID
	s.updatedAt = time.Now()
}

// AssignLineManager assigns a line manager
func (s *Starter) AssignLineManager(managerID *int64) {
	s.lineManagerID = managerID
	s.updatedAt = time.Now()
}

// CanBeDeleted checks if starter can be deleted (no dependencies)
...
// MarkAsRemoved records a removal event before deletion
