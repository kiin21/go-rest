...
import (
	...
	presentationDto "github.com/kiin21/go-rest/internal/starter/presentation/http/dto"
...
func (sh *StarterHandler) ListStarters(ctx *gin.Context) (res interface{}, err error) {
	...
	if result.Pagination.Prev != nil {
		prevQuery := ctx.Request.URL.Query()
		prevQuery.Set("page", *result.Pagination.Prev)
		prevURL := sh.urlResolver.AbsoluteURL(ctx, ctx.Request.URL.Path, prevQuery)
		result.Pagination.Prev = &prevURL
	}
	if result.Pagination.Next != nil {
		nextQuery := ctx.Request.URL.Query()
		nextQuery.Set("page", *result.Pagination.Next)
		nextURL := sh.urlResolver.AbsoluteURL(ctx, ctx.Request.URL.Path, nextQuery)
...
func (sh *StarterHandler) CreateStarter(ctx *gin.Context) (res interface{}, err error) {
	...
	enrichedData, err := sh.service.EnrichStarters(ctx, []*starterAggregate.Starter{starter})
	if err != nil {
...
func (sh *StarterHandler) Find(ctx *gin.Context) (res interface{}, err error) {
	...
	// Enrich with related data
	enrichedData, err := sh.service.EnrichStarters(ctx, []*starterAggregate.Starter{result})
	if err != nil {
		// Log error but continue with basic data
	}
