...
import (
	...
	presentationDto "github.com/kiin21/go-rest/internal/organization/presentation/http/dto"
	"github.com/kiin21/go-rest/pkg/httpctx"
	"github.com/kiin21/go-rest/pkg/response"
)

type OrganizationHandler struct {
	orgService  *application.OrganizationApplicationService
...
func (h *OrganizationHandler) ListDepartments(ctx *gin.Context) (res interface{}, err error) {
	...
	// Convert Presentation DTO → Application Keyword
	...
	if result.Pagination.Prev != nil {
		...
		prevURL := h.urlResolver.AbsoluteURL(ctx, ctx.Request.URL.Path, prevQuery)
	...
	if result.Pagination.Next != nil {
		...
		nextURL := h.urlResolver.AbsoluteURL(ctx, ctx.Request.URL.Path, nextQuery)
