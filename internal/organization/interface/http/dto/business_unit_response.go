...
// FromBusinessUnit converts domain entity to response DTO.
func FromBusinessUnit(unit *domain.BusinessUnit) *BusinessUnitResponse {
	if unit == nil {
		return nil
	}
...
// FromBusinessUnits converts a slice of business units to DTOs.
func FromBusinessUnits(units []*domain.BusinessUnit) []*BusinessUnitResponse {
	responses := make([]*BusinessUnitResponse, len(units))
	for i, unit := range units {
		responses[i] = FromBusinessUnit(unit)
	}
	return responses
}
